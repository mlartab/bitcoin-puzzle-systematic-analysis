...
|--------------------|----------|-----------------------------------------------------------------|
| `seed_hunt_all.py` | Python   | Tests passphrases against BIP32 wallets (4 derivation paths)   |
| `seed_hunt_sha256.py` | Python | Tests passphrases against old-school SHA256 brainwallets      |
| `z3_crack.py`      | Python   | Uses Z3 SAT solver to reverse-engineer the Mersenne Twister    |
| `gen_observed.py`  | Python   | Generates partial output lists for the untwister tool          |
| `mt_brute.cpp`     | C++      | Scans ALL 4 billion possible 32-bit seeds for Python's `random`|
| `rand_brute.cpp`   | C++      | Scans ALL 4 billion possible seeds for C's `rand()`            |
| `java_crack.cpp`   | C++      | Recovers Java's Random seed via modular arithmetic (no brute force!) |
| `scanner.py`       | Python   | Custom range scanner to sweep through key ranges               |
...
Keyhunt also supports:
- **BSGS mode** (Baby-Step Giant-Step) — uses more RAM but can search faster for smaller bit ranges
- **Address mode** — can verify private keys against known addresses

> ⚠️ Keyhunt is included in this repo as source code only (in `tools/keyhunt/`). The original is by [@albertobsd](https://github.com/albertobsd). All credit goes to them.
...
When a BIP32 wallet is created from a seed phrase, it generates two things:
...
Many scripts from 2015 used `random.seed(some_number)`. Python's `random` uses the Mersenne Twister (MT19937) internally. I scanned all 4,294,967,296 possible 32-bit seeds in C++. For each seed, I generated 69 keys and checked against the known ones.
...
- [**AlbertoBSD**](https://github.com/albertobsd) — for the incredible `keyhunt` tool
- [**Jean-Luc PONS**](https://github.com/JeanLucPons) — for the Kangaroo solver
